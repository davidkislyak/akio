...
def isval(uid:str, key:str):
   if uid: return True if db.get(uid) and db[uid].get(key) else False
   else: return True if db.get(key) else False
...
def addval(uid:str, key:str, val:int, start:int=0):
   if uid:
      if not db.get(uid): db[uid] = {}
      db[uid][key] = db[uid][key] + val if db[uid].get(key) else start + val
   else: db[key] += val if db.get(key) else start + val

def getval(uid:str, key:str, default=0):
   val = db[uid].get(key) if uid and db.get(uid) else db.get(key)
...
async def autosave():
   ...
   while not bot.is_closed:
      await asyncio.sleep(db["save-interval"])
...
async def on_message(msg):
   ...
   time, timer = dt.now(), getval("", "timer", False)
   ...
   if random.random() < getval("", "drop-chance", 0.3):
   ...
   if cmd == "status" and args:
      ...
      await reply(f"{uname}, you have **{getval(uid,'gold',default=0)}**:large_orange_diamond:")

   elif cmd == "cookies":
      await reply(f"{uname}, you have **{getval(uid,'cookies',default=0)}** :cookie:.")
   ...
   # admin commands
      ...
      if cmd in ("rst", "restart"):
         ...
         os.startfile(getval("", "filepath", "C:\\Users\\Admin\\Documents\\Python\\bot\\bot.py"))
         await bot.logout()
         sys.exit(0)
